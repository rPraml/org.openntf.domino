...
import org.openntf.tasklet.Scheduler;

public class PeriodicScheduler implements Scheduler {
	private static final Logger log_ = Logger.getLogger(PeriodicScheduler.class.getName());
	...
	public void eventStop() {
		if (periodSecond < 0) {
			synchronized (nextExecTime) {
				nextExecTime.setTimeInMillis(System.currentTimeMillis());
				nextExecTime.add(Calendar.SECOND, -periodSecond);
				shiftToNextTimeWindow();
			}
		}
